...
export function createDataLoader<S, K, V>(
  ...
  let service: S;
  const cacheMap =
    options.cacheMap ?? new TTLCache({ max: env.dataloaderCache.max, ttl: env.dataloaderCache.ttlInMilliseconds });
  try {
...
export const createReqContext = async (req: Request, res: Response, cache: Redis): Promise<GraphqlContext> => {
