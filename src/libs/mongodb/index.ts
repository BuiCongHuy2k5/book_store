...
): ModelType<T, QueryHelpers> {
  const db = mongoose.connection.useDb(databaseName, { useCache: true, noListener: true });

  const DbModel = getModel(db, model) as unknown as ModelType<T, QueryHelpers>;
...
export function getModelByType<T extends AnyParamConstructor<any>, QueryHelpers = BeAnObject>(
  ...
  return db.model(model.modelName, model.schema) as unknown as ReturnModelType<T, QueryHelpers>;
}
